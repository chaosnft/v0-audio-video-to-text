...
import { useRouter } from "next/navigation"  // Thêm để manual redirect
...
export default function LoginPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    if (result?.error) {
      ...
      router.push("/")  // Manual redirect về homepage
    }
    ...
    <div className="min-h-screen bg-black text-white flex items-center justify-center p-6">
      <Card className="w-full max-w-md bg-white/5 border-white/10">
        <CardHeader className="space-y-1">
          ...
          <p className="text-center text-white/60">Welcome back. Enter your credentials.</p>
        </CardHeader>
        <CardContent className="space-y-4">
          {error && <div className="bg-red-900/20 border border-red-500/30 text-red-400 px-4 py-2 rounded-lg text-sm">{error}</div>}
